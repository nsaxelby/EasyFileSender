...
namespace FluentFTP {
	public partial class FtpClient : IDisposable {
		...
		/// <param name="inputFileLengthBytes">Full or relative path of the file</param>
		/// <returns>A stream for writing to the file on the server</returns>
		/// <example><code source="..\Examples\OpenWrite.cs" lang="cs" /></example>
		public Stream OpenWrite(string path, long inputFileLengthBytes) {
			return OpenWrite(path, FtpDataType.Binary, true, inputFileLengthBytes);
		}
		...
		/// <param name="path">Full or relative path of the file</param>
		/// <param name="type">ASCII/Binary</param>
		/// <param name="inputFileLengthBytes">Expected file size from source</param>
		/// <returns>A stream for writing to the file on the server</returns>

		/// <example><code source="..\Examples\OpenWrite.cs" lang="cs" /></example>
		public virtual Stream OpenWrite(string path, FtpDataType type, long inputFileLengthBytes) {
			return OpenWrite(path, type, true, inputFileLengthBytes);
		}
		...
		/// <returns>A stream for writing to the file on the server</returns>
		/// <example><code source="..\Examples\OpenWrite.cs" lang="cs" /></example>
		public virtual Stream OpenWrite(string path, FtpDataType type, bool checkIfFileExists, long inputFileLengthBytes) {
			// verify args
			if (path.IsBlank()) {
				throw new ArgumentException("Required parameter is null or blank.", "path");
			}
			...
			lock (m_lock) {
				...
				client.SetDataType(type);
				length = checkIfFileExists ? client.GetFileSize(path) : 0;
				// STORE FILNAME longBytesFileSize
				// Seperators are first space, and last space.
				stream = client.OpenDataStream("STOR " + path.GetFtpPath() + " " + inputFileLengthBytes, 0);
		...
		public IAsyncResult BeginOpenWrite(string path, long inputFileLengthBytes, AsyncCallback callback, object state) {
			return BeginOpenWrite(path, FtpDataType.Binary, inputFileLengthBytes, callback, state);
		}

		private delegate Stream AsyncOpenWrite(string path, FtpDataType type, long inputFileLengthBytes);
		...
		/// <returns>IAsyncResult</returns>
		/// <example><code source="..\Examples\BeginOpenWrite.cs" lang="cs" /></example>
		public IAsyncResult BeginOpenWrite(string path, FtpDataType type, long inputFileLengthBytes, AsyncCallback callback, object state) {
			AsyncOpenWrite func;
			IAsyncResult ar;

			lock (m_asyncmethods) {
				ar = (func = new AsyncOpenWrite(OpenWrite)).BeginInvoke(path, type, inputFileLengthBytes, callback, state);
				m_asyncmethods.Add(ar, func);
			}
		...
		/// <returns>A stream for writing to the file on the server</returns>
		public virtual async Task<Stream> OpenWriteAsync(string path, FtpDataType type, bool checkIfFileExists, long inputFileLengthBytes, CancellationToken token = default(CancellationToken)) {
			// verify args
			if (path.IsBlank()) {
				throw new ArgumentException("Required parameter is null or blank.", "path");
			}
			...
			await client.SetDataTypeAsync(type, token);
			length = checkIfFileExists ? await client.GetFileSizeAsync(path, token) : 0;
			stream = await client.OpenDataStreamAsync("STOR " + path.GetFtpPath() + " " + inputFileLengthBytes, 0, token);
		...
		/// <param name="token">Cancellation Token</param>
		/// <returns>A stream for writing to the file on the server</returns>
		public Task<Stream> OpenWriteAsync(string path, FtpDataType type, long inputFileLengthBytes, CancellationToken token = default(CancellationToken)) {
			return OpenWriteAsync(path, type, true, inputFileLengthBytes, token);
		}
		...
		/// <param name="token">Cancellation Token</param>
		/// <returns>A stream for writing to the file on the server</returns>
		public Task<Stream> OpenWriteAsync(string path, long inputFileLengthBytes, CancellationToken token = default(CancellationToken)) {
			return OpenWriteAsync(path, FtpDataType.Binary, true, inputFileLengthBytes, token);
		}
#endif
